...
namespace ObservabilityValidator.Providers.Honeycomb;
...
public sealed class HoneycombProvider : IObservabilityProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var body = new
        {
            // Prefer explicit window over time_range to match Validator's from/to
            start_time = from.ToString("o"),
            end_time = to.ToString("o"),
            // Keep the query simple: request raw events ordered by time, limited
            filters,
        ...
        // Honeycomb query results often return an array under "results" with items containing a "data" object
        ...
        foreach (var item in resultsEl.EnumerateArray())
        {
            var data = item.TryGetProperty("data", out var dataEl) && dataEl.ValueKind == JsonValueKind.Object
                ? dataEl
                : item; // Some responses may be direct objects
            ...
            // Attempt to read common Honeycomb/OTel field names with graceful fallbacks
    ...
    public Task<IReadOnlyList<MetricPoint>> GetMetricsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Metrics not implemented yet for Honeycomb
    ...
    private static List<(string key, string value)> ParseSimpleQuery(string query)
    {
        ...
        // Tokenize on spaces, but allow values to include spaces by continuing until the next key:
        // key:value value-with-spaces nextkey:nextvalue
